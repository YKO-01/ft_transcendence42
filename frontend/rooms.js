...
class ManageGameAttachment {
    ...
    handleGameState(gameState) {
        ...
        if (gameState.players === 1) {
            ...
            this.player1.textContent = gameState.nickname;
            ...
            this.player2.textContent = this.letfPlayer ? this.player1.textContent : gameState.nickname;
...
async function fetch_on_next_around(roomID) {
    try {
        let response = await fetch(`http://localhost:8000/api/rooms/matches/${roomID}`, {
            method: 'GET',
            credentials: 'include',
            headers: {}
        });
